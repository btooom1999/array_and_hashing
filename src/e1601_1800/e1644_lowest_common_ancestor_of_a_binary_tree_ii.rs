...
fn helper(
    ...
    if let Some(node_rc) = node {
        ...
        let left = helper(node.left.clone(), p, q, found);
        let right = helper(node.right.clone(), p, q, found);
        let current = if node.val == p || node.val == q { 1 } else { 0 } + left + right;
...
pub fn main() {
    ...
    // let q = Some(Rc::new(RefCell::new(TreeNode::new(10))));
